        ...
        <div className="container-max px-4 md:px-6">
          <ScrollObserver delay={0}>
            <div className="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
              <div>
                <p className="text-base uppercase tracking-[0.2em] text-lime">Welkom terug</p>
                <h1 className="font-playfair text-4xl md:text-5xl font-bold">{displayName}</h1>
                {rank && <p className="mt-1 text-base text-text2">Rank: <span className="text-lime font-semibold">{rank}</span></p>}
              </div>
              <div className="flex items-center gap-3">
                <div className="rounded-2xl border border-lime/30 bg-lime/10 px-5 py-3">
                  ...
                  {creditsError ? (
                    <p className="text-sm text-text2">Niet beschikbaar</p>
                  ) : (
                ...
                <Link
                  ...
                  className="rounded-2xl border border-border px-4 py-3 text-base font-bold text-text2 hover:text-text hover:border-muted transition-colors"
                >
                  + Bijkopen
                </Link>
              </div>
          ...
          {creditsError && (
            <div className="mb-6 rounded-lg border border-yellow-500/30 bg-yellow-500/10 p-3 text-base text-yellow-400">
          ...
          {owedSplits.length > 0 && (
            <ScrollObserver delay={0.05}>
              <div className="mb-6 rounded-2xl border border-lime/30 bg-lime/5 p-5">
                <h2 className="font-bold text-xl mb-1">Openstaande verzoeken</h2>
                <p className="text-text2 text-base mb-4">Iemand heeft een baan voor je geboekt — jouw aandeel:</p>
                <div className="space-y-2">
                  {owedSplits.map((s) => {
                    ...
                    return (
                      <div key={s.id} className="flex flex-wrap items-center gap-3 rounded-xl border border-border/70 bg-dark/60 p-3 text-base">
                        <span className="min-w-0">
                          <span className="block font-semibold text-text truncate">{s.clubName} · {s.courtName}</span>
                          <span className="block text-text3 text-sm">
                            {new Date(s.date + "T12:00:00").toLocaleDateString("nl-NL", { day: "numeric", month: "short" })} · {s.startTime}–{s.endTime}
                          </span>
                          <span className="block text-text3 text-sm mt-0.5">Geboekt door {bookerLabel}</span>
                          {others.length > 0 && <span className="block text-text3 text-sm">Ook mee: {others.join(", ")}</span>}
                        </span>
                        ...
                        <button
                          ...
                          className="bg-lime text-dark px-3 py-1.5 rounded-full text-sm font-bold hover:opacity-90 disabled:opacity-50"
                        >
          ...
          <div className="grid lg:grid-cols-[1.4fr_0.6fr] gap-6">
              ...
              <ScrollObserver delay={0.1}>
                <div className="border border-border rounded-2xl p-6 bg-surface/40 mb-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="font-bold text-xl">Aankomende boekingen</h2>
                    <Link href="/clubs" className="text-base text-lime hover:text-text transition-colors">
                      + Nieuwe boeking
                    </Link>
                  </div>
                  {upcoming.length === 0 ? (
                    <p className="text-base text-text2">Nog geen aankomende boekingen. Reserveer je eerste baan!</p>
                  ) : (
                        ...
                        return (
                          <div key={b.id} className="flex flex-wrap items-center gap-3 rounded-2xl border border-border/70 bg-dark/60 p-4">
                            <div className="min-w-0">
                              ...
                              {isBooker && !canCancel && <p className="text-sm text-yellow-400 mt-0.5">Annuleren kan niet meer (binnen 12 uur voor starttijd)</p>}
                              {geoResult[b.id] && <p className="text-sm text-text2 mt-0.5">{geoResult[b.id]}</p>}
                            </div>
                            <div className="ml-auto flex items-center gap-3 flex-none">
                              ...
                              {isBooker && qrEnabledClubIds.has(b.clubId) && !b.checkedInAt && (
                                <button
                                  ...
                                  className="text-sm border border-lime/40 text-lime rounded-full px-3 py-1.5 hover:bg-lime/10 disabled:opacity-50"
                                >
                              ...
                              {canCancel && (
                                <button
                                  ...
                                  className="text-sm border border-red-500/30 text-red-400 rounded-full px-3 py-1.5 hover:bg-red-500/10 disabled:opacity-50"
                                >
            ...
            <ScrollObserver delay={0.2}>
              <div className="border border-border rounded-2xl p-6">
                <h2 className="font-bold text-xl mb-4">Snelle acties</h2>
                <div className="space-y-3">
                  <Link href="/clubs" className="block w-full bg-lime text-dark py-2 rounded-lg font-bold text-center hover:opacity-90 transition-opacity text-base">
                    Baan reserveren
                  </Link>
                  <Link href="/betalen?tab=credits" className="block w-full border border-border text-text2 hover:text-text py-2 rounded-lg font-bold text-center transition-colors text-base">
                    Credits bijkopen
                  </Link>
                  <Link href="/betalen" className="block w-full border border-border text-text2 hover:text-text py-2 rounded-lg font-bold text-center transition-colors text-base">
                    Abonnement beheren
                  </Link>
                </div>
              </div>
            ...
            <ScrollObserver delay={0.25}>
              <div className="border border-border rounded-2xl p-6 mt-6">
                <h2 className="font-bold text-xl mb-4">Credits geschiedenis</h2>
                {ledger.length === 0 ? (
                  <p className="text-base text-text2">Nog geen mutaties.</p>
                ) : (
                  <div className="space-y-2">
                    {ledger.map((entry) => (
                      <div key={entry.id} className="flex items-center gap-2 text-base border-b border-border/30 pb-2 last:border-b-0 last:pb-0">
                        <div className="min-w-0 flex-1">
                          <p className="text-text truncate">{LEDGER_LABELS[entry.type] || entry.type}</p>
                          <p className="text-sm text-text3">{new Date(entry.createdAt).toLocaleDateString("nl-NL", { day: "numeric", month: "short" })}</p>
                        </div>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl border border-border bg-surface2 p-6" onClick={(e) => e.stopPropagation()}>
        <h3 className="font-bold text-xl mb-1">Hoe wil je inchecken?</h3>
        <p className="text-text2 text-base mb-5">Kies QR om te laten scannen bij de balie, of check in via je locatie als je al bij de club bent.</p>
        <div className="space-y-2">
          <button onClick={onChooseQr} className="w-full bg-lime text-dark py-2.5 rounded-lg font-bold text-base hover:opacity-90 transition-opacity">
            Toon QR-code
          </button>
          <button onClick={onChooseLocation} className="w-full border border-lime/40 text-lime py-2.5 rounded-lg font-bold text-base hover:bg-lime/10 transition-colors">
            Check in via locatie
          </button>
        </div>
        <button onClick={onClose} className="w-full mt-4 text-text2 hover:text-text py-1 text-base transition-colors">
          Annuleren
        </button>
      </div>
    </div>
...
function QrCodeModal({ booking, email, onClose }: { booking: Booking; email: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        <p className="font-playfair text-xl font-bold mb-1">{booking.clubName}</p>
        <p className="text-base text-text2 mb-4">
          {booking.courtName} ·{" "}
        ...
        <p className="mt-4 font-mono text-base text-lime">{booking.bookingCode}</p>
        <p className="text-sm text-text3 mt-1">Laat dit scannen bij de balie.</p>
        <button onClick={onClose} className="mt-5 w-full border border-border text-text2 hover:text-text py-2 rounded-lg font-bold text-base transition-colors">
          Sluiten
        </button>
      </div>
    </div>
...
function ProfileDetailsCard({ email }: { email?: string }) {
  ...
  const field = "w-full bg-dark border border-border rounded-lg px-3 py-2 text-base focus:outline-none focus:border-lime transition-colors"

  return (
    <div id="mijn-gegevens" className="border border-border rounded-2xl p-6 scroll-mt-24">
      <div className="flex items-center justify-between mb-1">
        <h2 className="font-bold text-xl">Mijn gegevens</h2>
        {!loadError && details && !editing && (
          <button onClick={() => setEditing(true)} aria-label="Gegevens bewerken" className="flex items-center gap-1.5 group">
            ...
            <span className="text-text3 group-hover:text-text2 transition-colors text-sm font-semibold">Bewerken</span>
          </button>
        )}
      </div>
      {email && (
        <div className="mb-4">
          ...
          <p className="text-base text-text2">{email}</p>
        </div>
      )}
      {!loadError && details && (
        <>
          {editing ? (
            <>
              <p className="text-sm text-text3 mb-4">Optioneel — telefoonnummer en adres, handig als je met Google bent ingelogd.</p>
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <button
                    onClick={handleSave}
                    disabled={saving}
                    className="flex-1 bg-lime text-dark py-2 rounded-lg font-bold text-base hover:opacity-90 transition-opacity disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    disabled={saving}
                    className="flex-1 border border-border text-text2 hover:text-text py-2 rounded-lg font-bold text-base transition-colors disabled:opacity-50"
                  >
                    Annuleren
                  </button>
                </div>
            ...
            <div className="space-y-2 text-base">
              ...
              {saved && <p className="text-lime text-sm pt-1">Opgeslagen ✓</p>}
            </div>
          )}
        </>
      )}
